...
```{r}
oj <- oj %>% 
  fastDummies::dummy_cols(select_columns = "purchase", remove_first_dummy = TRUE, remove_selected_columns = TRUE) %>% 
  mutate(quarter_of_purchase = case_when(weekof_purchase <= 240 ~ "Q1",
                                     weekof_purchase <= 253 ~ "Q2",
                                     weekof_purchase <= 266 ~ "Q3",
                                     TRUE ~ "Q4")) %>% 
  mutate(quarter_of_purchase = as.factor(quarter_of_purchase), 
         store_id = factor(store_id, levels = c(1, 2, 3, 4, 7)), 
         purchase_MM = as.factor(purchase_MM),
         special_mm = as.factor(special_mm),
         special_ch = as.factor(special_ch)) %>% 
  select(-c(store7, weekof_purchase, disc_ch, disc_mm, price_diff, list_price_diff, store)) 
```
...
```{r}
train_index <- createDataPartition(oj$purchase_MM, p = 0.8, list = FALSE, times = 1)
