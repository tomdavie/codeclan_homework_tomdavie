...
```{r}
# Backpack weight increases more with body weighgt in females than males
...
```{r}
back_problems_ordered <- backpack %>% 
  arrange(back_problems)

ggplot(back_problems_ordered, aes(x = "")) + 
  geom_col(aes(y = length(back_problems)/100, fill = back_problems), show.legend = FALSE) +
...
# There is a much higher percentage of back problems in men than women

```{r}
ggplot(back_problems_ordered, aes(x = "")) + 
  geom_col(aes(y = length(back_problems)/100, fill = back_problems, sex), show.legend = FALSE) +
  labs(x = "Sex",
       y = "Count",
...
```{r}
# This graph suggests if you have a high backpack weight to body weight ratio then you're far more likely to have back problems if you are male. This is true in females but far less significant. 
ggplot(backpack) +
  geom_col(aes(x = back_problems, y = ratio, fill = back_problems), show.legend = FALSE) +
  facet_grid(~sex) +
  labs(x = "Back Problems",
       y = "Backpack Weight to Body Weight Ratio",
       title = "\nThe Impact of Backpack Weight to Body Weight Ratio on Back Problems between the sexes\n")
```
